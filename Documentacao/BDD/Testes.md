...
* **Dado** que o repositório contém arquivos de testes prefixados com `Teste`;
* **Quando** o arquivo centralizador [ExecutarTestes.py](file:///C:/Users/Eduardo/Documents/SistemaOperacionalFSO-V3/ExecutarTestes.py) é executado na raiz do projeto;
...
* **Quando** o escalonador executa a rotina de envelhecimento (`self._envelhecimento()`);
* **Então** o processo mais antigo da fila de prioridade 3 deve ser promovido para a fila de prioridade superior (prioridade 2);
...
* **Então** o quadro deve ser preenchido, mas o contador de `faltasPagina` do respectivo processo deve continuar em zero.
...
* **E** substituí-lo pela nova página, incrementando corretamente o contador de faltas de página.
...
### História de Usuário:
Como testador do GerenciadorRecurso,
...
## 5. Testes do Sistema de Arquivos (Disco e First-Fit)
...
#### Cenário 3: Controle de Quantum e Atomicidade na Escrita de Arquivos
...
* **Então** a operação deve falhar com erro de preempção;
* **E** nenhuma alteração (0 blocos escritos) deve ser feita no disco lógico para garantir a atomicidade das System Calls de escrita.
...
* **Então** a função de utilidades deve lançar `ErroEntrada` impedindo a conversão de valores diferentes de representações estritamente booleanas equivalentes a 0 ou 1.
...
* **Então** o parser deve rejeitar o processo com `ErroEntrada` indicando que o tempo limite de segurança (1.000.000) foi excedido.
...
* **Quando** a função `_validarNome` valida a chamada;
* **Então** o disco lógico deve recusar a gravação lançando `ErroSistemaArquivos` para impedir a quebra de layout na impressão final do mapa de ocupação do disco.
...
* **Quando** o Processo 1 realiza consecutivas faltas de páginas locais e substituições de quadros via LRU local;
* **Então** a contagem de faltas de páginas e a ordem do LRU dos quadros do Processo 2 devem permanecer intocadas e isoladas, sem sofrer qualquer interferência ou leak.
...
* **Então** o [GerenciadorRecurso](file:///C:/Users/Eduardo/Documents/SistemaOperacionalFSO-V3/Gerenciadores/GerenciadorRecurso.py) deve acusar que o PID 2 não possui alocações ativas no dicionário interno;
* **E** levantar uma exceção `ErroGerenciadorRecurso`, protegendo as cotas de hardware contra invasões lógicas.
